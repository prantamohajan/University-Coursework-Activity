...
function validateProduct(body, { partial = false } = {}) {
  const errors = [];
  const fields = ["name", "category", "era", "price", "stock", "description", "image"];
...
app.get("/", (req, res) => {
  res.send("Antique Store API is running. Try GET /api/products");
});

const PORT = process.env.PORT || 5000;
app.listen(PORT, () => {
  console.log(`Antique Store API listening on http://localhost:${PORT}`);
});
